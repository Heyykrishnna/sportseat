      ...
      <div className="grid gap-6 lg:grid-cols-[0.58fr_1fr]">
        ...
        <div className="grid gap-4 sm:grid-cols-[1fr_0.55fr]">
          <div className="overflow-hidden rounded-[8px] bg-[#6fb1d2]">
            <img className="h-full min-h-[280px] w-full object-cover object-center" src={tennisAction} alt="Tennis player jumping for a shot" />
          </div>
          <div className="grid gap-4">
            {competitionCards.map((card) => (
              <article key={card.title} className="relative min-h-[160px] overflow-hidden rounded-[8px] text-white">
                ...
                <div className="relative flex h-full min-h-[160px] flex-col justify-end p-5">
                  ...
                  <h3 className="mt-2 text-sm font-black">{card.title}</h3>
                  <p className="mt-1 text-xs font-semibold leading-5 text-white/82">{card.text}</p>
                </div>
              </article>
            ))}
          </div>
        ...
        {bookingCards.map((card) => (
          ...
          >
            ...
            <div className="relative flex min-h-[420px] flex-col justify-between p-6">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-white/70">{card.sport}</p>
                <h3 className="mt-3 text-2xl font-black leading-[1.05] text-white sm:text-3xl">{card.title}</h3>
                <p className="mt-4 text-sm font-light text-white/78">{card.venue}</p>
              </div>
